...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    ...
    ctrans_tosearch = uf.convert_color(img_tosearch, conv='RGB2LUV')
    if scale != 1:
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            #hog_correct = filter(lambda x: x != nan, hog_features)
            #hog_features = np.hstack([x for x in hog_features if not np.isnan(x)])
